...
.khz-toggle { position: fixed; bottom: 20px; left: 20px; width: 40px; height: 40px; background: #000; border: 2px solid #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 100000; color: #fff; font-size: 20px; font-weight: bold; box-shadow: 0 0 10px #fff; font-family: sans-serif; transition: 0.3s; }
.khz-toggle:hover { background: #fff; color: #000; }
.khz-panel { position: fixed; top: 100px; left: 100px; width: 300px; background: rgba(0, 0, 0, 0.95); border-radius: 16px; padding: 20px; z-index: 99999; color: #fff; font-family: sans-serif; box-shadow: 0 0 20px rgba(255, 255, 255, 0.6); cursor: grab; display: none; border: 2px solid #fff; } /* Added white border */
...
.khz-button { display: block; width: 100%; margin: 10px 0; padding: 10px; background: #000; color: white; border: 2px solid #fff; border-radius: 8px; cursor: pointer; font-size: 14px; transition: 0.3s; } /* Black background, white border and text */
.khz-button:hover { background: #fff; color: #000; border-color: #000; } /* Hover: White background, black text and border */
.khz-button.active{background:#fff;color:#000;border-color:#000;box-shadow:0 0 8px rgba(255, 255, 255, 0.8);} /* Active: White background, black text and border, white glow */
...
.khz-input-group input { width: 60px; background: #333; color: #fff; border: 1px solid #fff; border-radius: 4px; padding: 4px; text-align: center; } /* Darker background for input, white text and border */
.khz-toast{position:fixed;bottom:20px;right:20px;background:#000;color:#fff;border:1px solid #fff;border-radius:8px;padding:12px 16px;margin-top:10px;box-shadow:0 0 10px rgba(255, 255, 255, 0.8);font-size:14px;font-family:sans-serif;z-index:999999;animation:fadeIn 0.3s ease-out;overflow:hidden;width:fit-content;max-width:300px} /* Toast: black background, white border and shadow */
.khz-toast.hide{animation:fadeOut 0.5s ease forwards}
.khz-toast-progress{position:absolute;left:0;bottom:0;height:4px;background:#fff;animation:toastProgress linear forwards;animation-duration:4s;width:100%} /* Toast progress: white */
...
document.head.appendChild(style);
    ...
    if (features.questionSpoof && originalResponse.ok) {
        ...
        try {
            ...
            if (responseObj?.data?.assessmentItem?.item?.itemData) {
                const phrases = ["1nstagram dev", "fuck everyone", "🥀", "fuck gov."];
  ...
  splash.textContent = "KHAN FUCKER";
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Initialize button states based on 'features'
        document.getElementById('khz-btn-question').classList.toggle('active', features.questionSpoof);
        document.getElementById('khz-btn-video').classList.toggle('active', features.videoSpoof);
        document.getElementById('khz-btn-reveal').classList.toggle('active', features.revealAnswers);
        document.getElementById('khz-btn-auto').classList.toggle('active', features.autoAnswer);
        document.getElementById('khz-btn-dark').classList.toggle('active', features.darkMode);
        document.getElementById('khz-btn-rgb').classList.toggle('active', features.rgbLogo);
        document.getElementById('khz-btn-oneko').classList.toggle('active', features.oneko);
        ...
        features.oneko = false; // Ensure oneko is off by default
...
function toggleRgbLogo() {
  ...
  if (features.rgbLogo) {
    ...
    khanLogo.classList.remove("force-rgb-logo"); // Ensure class is removed
